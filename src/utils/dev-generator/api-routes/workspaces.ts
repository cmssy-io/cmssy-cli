...
function loadEnvConfig(): { apiUrl: string; apiToken: string | null; workspaceId: string | null } {
  const fs = require("fs");
...
export async function GET() {
  ...
  try {
    ...
    if (config.workspaceId) {
      targetWorkspace = workspaces.find((w: any) => w.id === config.workspaceId);
    }
    if (!targetWorkspace && workspaces.length > 0) {
      targetWorkspace = workspaces[0];
    }

    // Fetch published blocks for target workspace
